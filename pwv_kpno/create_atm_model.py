...
files generated by the LSST projects phosim code and the Beer-Lambert Law. It
is heavily based on code written by Wei Hu in numIntwei_new.py.

This code does not to be run by the user. The models generated by this code 
are already included in the pwv_kpno package. This code is included for
documentation purposes and for reference in future package development.
"""
...
def _load_cross_section(filename, x_fine):
    ...
    yfun = interpolate.interp1d(cs[:, 0], cs[:, 1], kind='nearest')
    cs = np.transpose(np.array([x_fine, yfun(x_fine)]))
...
def _calculate_atm(atm_list, x_fine, xlf_dict, pint_list):
    ...
    """
    # formula, x_fine in um / 0.5um # aerosoltau
...
def write_atm_models(output_dir):
    ...
    output_dir. CSV files are named 'atm_trans_mod_XXX_pwv' where XXX
    ...
    xlf_dict = {'h2o': np.arange(0.1, 31, 1) * mol_cm_3,  # 0.1 - 30.1 mm in units of mol/cm^3
                ...
                'o2': np.array([1.]),  # 0.02%
                'tau': np.array([0.05]),  # aerosoltau
                'index': np.array([1.28])}  # aerosolindex
